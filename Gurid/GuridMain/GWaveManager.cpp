...
bool GWaveManager::Frame(){
	...
	if (WAVE_ST_NEWENEMY_ADD == g_pMain->m_GameSingle.m_nWavePhase) {
		//ToDo. 적추가 한다 wave카운트+2 //첫판은 적 탱크 3대

		g_pMain->m_GameSingle.m_nWaveBeforePhase = WAVE_ST_NEWENEMY_ADD;
	...
	if (WAVE_ST_GAMEOVER == g_pMain->m_GameSingle.m_nWavePhase) {
		...
		g_pMain->m_GameSingle.m_nWavePhase = WAVE_ST_TO_MENU;
	}
