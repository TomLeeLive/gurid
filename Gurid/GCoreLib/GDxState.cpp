...
namespace DX
{
	ID3D11RasterizerState* GDxState::g_pRSWireFrame = 0;
	ID3D11RasterizerState* GDxState::g_pRSSolid = 0;
	ID3D11RasterizerState* GDxState::g_pRSBackCullSolid = 0;
	ID3D11RasterizerState* GDxState::g_pRSNoneCullSolid = 0;		
	ID3D11RasterizerState* GDxState::g_pRSFrontCullSolid = 0;
	ID3D11RasterizerState* GDxState::g_pRS[3] = { 0, };
	
	ID3D11SamplerState*	 GDxState::g_pTexSS = 0;
	ID3D11SamplerState*  GDxState::g_pSSWrapLinear = NULL;
	ID3D11SamplerState*  GDxState::g_pSSWrapPoint = NULL;
	ID3D11SamplerState*  GDxState::g_pSSMirrorLinear = NULL;
	ID3D11SamplerState*  GDxState::g_pSSMirrorPoint = NULL;
	ID3D11SamplerState*  GDxState::g_pSSClampLinear = NULL;
	ID3D11SamplerState*  GDxState::g_pSSClampPoint = NULL;
	ID3D11SamplerState*  GDxState::g_pSS[6] = { 0, };

	ID3D11DepthStencilState*	GDxState::g_pDSSDepthEnable = 0;
	ID3D11DepthStencilState*	GDxState::g_pDSSDepthDisable = 0;
	ID3D11DepthStencilState*	GDxState::g_pDSSDepthEnableNoWrite = 0;
	ID3D11DepthStencilState*	GDxState::g_pDSSDepthDisableNoWrite = 0;
	ID3D11DepthStencilState*	GDxState::g_pDSSDepthStencilAdd =0;
	ID3D11DepthStencilState*	GDxState::g_pDSS[5] = { 0, };

	ID3D11BlendState*	  GDxState::g_pAlphaBlend = 0;
	ID3D11BlendState*	  GDxState::g_pNoAlphaBlend = 0;
	ID3D11BlendState*	  GDxState::g_pBSColorOne = 0;
	ID3D11BlendState*	  GDxState::g_pBSOneOne = 0;
	ID3D11BlendState*	  GDxState::g_pBSOneZero = 0;
	ID3D11BlendState*	  GDxState::g_pBSAlphaOne = 0;
	ID3D11BlendState*	  GDxState::g_pBSMaxOneOne = 0;
	ID3D11BlendState*	  GDxState::g_pBS[7] = { 0, };
	
	HRESULT GDxState::SetState(ID3D11Device*	pd3dDevice)
	{
		...
		pd3dDevice->CreateBlendState(&blendDesc, &GDxState::g_pBSColorOne);

		blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;// D3D11_BLEND_OP_REV_SUBTRACT;
		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;// DEST_COLOR;
		pd3dDevice->CreateBlendState(&blendDesc, &GDxState::g_pBSOneOne);

		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ZERO;// DEST_COLOR;
		pd3dDevice->CreateBlendState(&blendDesc, &GDxState::g_pBSOneZero);

		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
		pd3dDevice->CreateBlendState(&blendDesc, &GDxState::g_pBSAlphaOne);

		blendDesc.AlphaToCoverageEnable = TRUE;
		blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_MAX;
		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
		pd3dDevice->CreateBlendState(&blendDesc, &GDxState::g_pBSMaxOneOne);
		...
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &GDxState::g_pRSBackCullSolid))) return hr;


		rsDesc.FillMode = D3D11_FILL_WIREFRAME;
		rsDesc.CullMode = D3D11_CULL_BACK;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &GDxState::g_pRSWireFrame)))
			return hr;
		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_BACK;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &GDxState::g_pRSSolid)))
			return hr;

		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_NONE;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &GDxState::g_pRSNoneCullSolid)))
			return hr;


		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_FRONT;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &GDxState::g_pRSFrontCullSolid)))
			return hr;
