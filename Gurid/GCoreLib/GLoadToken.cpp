...
void GLoadToken::Tokenize(const T_STR& text, const T_STR& delimiters, OutputIterator first)
{
...
;
const TCHAR* GLoadToken::GetSearchString(PTCHAR szSearchData, bool bLoop)
{
...
const TCHAR* GLoadToken::GetNextTokenString()
{
...
const TCHAR* GLoadToken::GetCurrentTokenString()
{
	return m_pwcTokenData.c_str();
}

// 버퍼스트링안에서 선단에 있는 오브젝트를 찾아 해당 인덱스를 넘겨준다.
INT GLoadToken::SearchTokenArray(INT NumObjType, TCHAR** pObjectType, DWORD dwNexGObjectIndex)
{
	INT iIndex = -1;
	...
	while (IsLineCheck)
	{
		if (dwNexGObjectIndex > 0 && dwNexGObjectIndex - 1 <= m_dwTokenIndex)
		{
			break;
		}
